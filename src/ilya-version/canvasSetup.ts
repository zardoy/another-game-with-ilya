...
`
    ...
    type BlockSide = 'south' | 'west' | 'east' | 'north' | 'top' | 'bottom'
    ...
    const getTriangles = (blockPosition: Vec3): Record<BlockSide, [Triangle, Triangle]> => {
        ...
        const blockSidesTriangles = _.mapValues(blockSidesArr, squareCoordinatesArr => {
            ...
            const triangles = [
                ...
                [...squareCoordinates.slice(2), squareCoordinates[0]] as TrianglePoints,
    ...
    const blocks = [new Block(vec3(0, 0, 1))]
    const recalculateMesh = () => {
        for (const block of blocks) {
            const sideTriangles = getTriangles(block.position)
            const blockColor: Color = (block.position.x + (block.position.z % 2)) % 2 === 0 ? [1, 0, 0] : [0, 1, 0]
    ...
    const rotateCamera = (delta: Vector2) => {
        ...
        const delimeter = 500
        rz -= x / delimeter
        if (ry - y / delimeter > -1.54 && ry - y / delimeter < 1.54) ry -= y / delimeter
    }
