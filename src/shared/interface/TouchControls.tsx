...
let TouchControls: React.FC<ComponentProps> = ({ updateTouchMoving }) => {
    ...
    const updateMoving = (newState: boolean, buttonIndexOrActions: number | MovementAction[]) => {
        ...
        if (typeof buttonIndexOrActions === 'number') {
            ...
            // unstable
            movementActions =
                typeof movementActionRaw[0] === 'string' ? [movementActionRaw as MovementAction] : (movementActionRaw as MovementAction[])
        } else {
    ...
    return !touchSupported ? null : (
        ...
        >
            ...
            >
                ...
                <img src={circleButtonSrc} draggable={false} style={{ width: touchControlsSize, height: touchControlsSize, order: 1 }} />
                <div
                    className={css`
                        position: fixed;
                        left: 0;
